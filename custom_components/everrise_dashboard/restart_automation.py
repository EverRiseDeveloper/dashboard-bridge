"""Seeds the four automations that make both bridge AND dashboard-frontend
...
Four separate automations, not one, because "download" and "restart" have
very different risk profiles and shouldn't share a trigger, and because the
bridge (Python code, needs a restart to actually load, and whose own update
entity is HACS's to refresh — not ours) and the dashboard frontend (static
files in www/, live the moment they're written, polled by our own code)
don't behave the same way either:

1. Force-check bridge update — fires every 30 minutes and just asks HA to
   refresh update.everrise_dashboard_config_bridge_update. This entity is
   generated by HACS, not this integration, and HACS's own background
   refresh for a custom repository is documented as "only about every 48
   hours" — nowhere near responsive enough on its own. This is the one step
   in the whole pipeline nobody but Home Assistant itself can be relied on
   to do: at a real client's house nobody is going to be sitting there
   manually forcing a refresh every time a release goes out, so it has to
   happen on a schedule, unattended, from inside HA.
2. Auto-install bridge update — fires the instant that refreshed update
   entity reports a new release, and just calls `update.install` on it.
   Cheap and harmless to do immediately: it only writes files to disk, same
   as a client clicking Update themselves.
3. Restart-if-pending — fires at 3am, but only restarts if the bridge's
...
4. Auto-install dashboard frontend update — fires the instant update.py's
   own coordinator (polling every 15 minutes — see update.py, which this
   integration owns outright, so no separate force-check automation is
   needed for it) reports a new dashboard-dist build, and calls
   `update.install` on it. No restart companion needed here either:
   frontend_updater.install_latest() only swaps files in www/, which every
   subsequent page load just picks up — there's no Python import to go
   stale the way there is for the bridge itself.
...
SEEDED_AUTOMATIONS = [
    {
        "id": FORCE_CHECK_BRIDGE_AUTOMATION_ID,
        "alias": "Force-check bridge update",
        "description": (
            "Refreshes the bridge's HACS update entity every 30 minutes — HACS's own "
            "background scan for a custom repository can go up to ~48 hours between checks on "
            "its own, which isn't responsive enough for anyone to rely on unattended. This is "
            "what actually surfaces a new bridge release to \"Auto-install bridge update\" "
            "below without anyone needing to open Home Assistant and force a check by hand. "
            "Seeded automatically by the bridge on first setup; edit or delete it like any "
            "other automation — it's never re-created once it exists."
        ),
        "triggers": [{"trigger": "time_pattern", "minutes": "/30"}],
        "conditions": [],
        "actions": [{"action": "homeassistant.update_entity", "target": {"entity_id": BRIDGE_UPDATE_ENTITY}}],
        "mode": "single",
    },
    {
...
def _seed(automations_path: Path) -> bool:
    ...
    a client who deleted just one of the four still only gets that one
